...
export const GroupRideModalContent: React.FC<GroupRideModalContentProps> = ({
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center gap-2 text-cyan-600 border-b border-slate-200 pb-3">
        <Users className="w-5 h-5" />
        <h2 className="text-lg font-bold text-slate-900">Group Ride Sync</h2>
      </div>
      <p className="text-xs text-slate-500 -mt-2">Share secret 6-digit code or join fellow wingmen</p>
        ...
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <div className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 flex flex-col items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="font-mono text-4xl sm:text-5xl font-black text-cyan-700 tracking-[0.2em] bg-white px-6 py-2.5 rounded-2xl border border-cyan-500/40 shadow-inner">
                {rideCode}
              </span>
              <button
          ...
          <div className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-left">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] text-slate-400 uppercase font-mono">Instant Preview</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {MOCK_RIDERS.map((r, idx) => (
                ...
                >
                  ...
                  <p className="text-[10px] text-slate-500 truncate">{r.bike}</p>
                </button>
              ))}
            </div>
          </div>
    ...
    >
      <div className="flex flex-col gap-5">
        ...
        {activeTab === 'host' ? (
          <div className="flex flex-col items-center text-center gap-4">
            {/* 6-Digit Code Card */}
            <div className="w-full bg-slate-950/80 border border-slate-800 rounded-2xl p-5 flex flex-col items-center gap-3">
              <span className="text-xs uppercase tracking-widest text-slate-400 font-bold">
                Your 6-Digit Secret Ride Code
              </span>

              <div className="flex items-center gap-2">
                <span className="font-mono text-4xl sm:text-5xl font-black text-cyan-400 tracking-[0.2em] bg-slate-900 px-6 py-2.5 rounded-2xl border border-cyan-500/40 shadow-inner">
                  {rideCode}
                </span>
                <button
                  onClick={handleCopyCode}
                  className="p-3 bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white rounded-xl border border-slate-700 transition-colors cursor-pointer"
                  title="Copy Code"
                >
            ...
            {/* Simulated Incoming Request Trigger (For testing approval workflow in browser) */}
            <div className="w-full bg-slate-950/50 border border-slate-800/80 rounded-xl p-3.5 text-left">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                  <span>Test Host Approval Flow (Simulation)</span>
                </span>
                <span className="text-[10px] text-slate-500 uppercase font-mono">Instant Preview</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {MOCK_RIDERS.map((r, idx) => (
                  <button
                    ...
                    className="p-2 bg-slate-900 hover:bg-slate-800 border border-slate-700/70 hover:border-cyan-500/50 rounded-lg text-left transition-colors cursor-pointer"
                  >
                    <p className="text-xs font-bold text-slate-200 truncate">{r.name}</p>
                    <p className="text-[10px] text-slate-400 truncate">{r.bike}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          <form onSubmit={handleJoinSubmit} className="flex flex-col gap-4">
            <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 flex flex-col gap-3">
              ...
              <input
                ...
                onChange={(e) => setInputCode(e.target.value.replace(/\D/g, ''))}
                placeholder="e.g. 748291"
                className="w-full bg-slate-900 border border-slate-700 focus:border-cyan-500 rounded-xl px-4 py-3 text-2xl font-mono tracking-widest text-center text-cyan-400 font-bold focus:outline-none focus:ring-1 focus:ring-cyan-500"
                required
              />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setGuestName(e.target.value)}
                  placeholder="e.g. Liam Ross"
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-sm text-slate-100 focus:border-cyan-500 focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Your Bike Model</label>
                <input
                  type="text"
                  value={guestBike}
                  onChange={(e) => setGuestBike(e.target.value)}
