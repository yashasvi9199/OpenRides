...
interface EmergencyQRModalContentProps {
  user: UserProfile;
  onOpenPublicView: () => void;
}

export const EmergencyQRModalContent: React.FC<EmergencyQRModalContentProps> = ({
  user,
  onOpenPublicView,
}) => {
  ...
  return (
    <div className="flex flex-col items-center gap-5 text-center">
      <div className="w-full flex items-center gap-2 text-red-500 border-b border-slate-200 pb-3">
        <QrCode className="w-5 h-5" />
        <h2 className="text-lg font-bold text-slate-900">Emergency Medical QR Badge</h2>
      </div>
      <p className="text-xs text-slate-500 -mt-2">Scan directly from helmet, fuel tank, or jacket sticker</p>

      {/* Printable/Scannable Badge Container */}
      <div className="bg-slate-900 border-2 border-red-500 rounded-3xl p-6 w-full flex flex-col items-center gap-3 shadow-xl relative overflow-hidden">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-2xl flex flex-col items-center my-1 border-4 border-slate-950">
          <QRCodeSVG
            value={publicUrl}
            size={180}
            level="H"
        ...
        <div className="w-full bg-slate-950 rounded-xl p-2.5 text-xs text-slate-300 flex items-center justify-between border border-slate-800">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-2.5">
        ...
        <Button
          variant="secondary"
          leftIcon={<ExternalLink className="w-4 h-4 text-cyan-600" />}
